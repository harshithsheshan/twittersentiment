...
def wait():
    ...
    sdate= request.form['sd']
    edate= request.form['ed']
    m = request.form['m']
    ...
    #startDate=to_datetime(sdate, format='%d-%m-%Y', errors='coerce')
    ...
    for tweet in tweepy.Cursor(api.search, q=s, lang="en").items(int(m)):
        if (tweet.created_at < endDate and tweet.created_at > startDate):
            ...
            if (get_sentiment(tweet.text) == "positive"):
                ...
                #for w in words:
                 #   if w not in stopWords and w not in punctuations:                            #print(w)
    ...
    return render_template('result.html',posperc=posperc,negperc=negperc,nutperc=nutperc)
